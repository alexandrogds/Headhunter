...
url = 'https://headhuntersbrazil.com/pt-br/'
...
with open('headhuntersbrazil.html', 'w', encoding='utf-8') as file:
    file.write(html_content)

# Parse the HTML content
soup = BeautifulSoup(html_content, 'html.parser')

# Extract the URL from the specified <section> element
job_links = []
sections = soup.find_all('section', {'class': 'links masonry-brick'})
for section in sections:
    links = section.find_all('a', href=True)
    for link in links:
        job_links.append(link['href'])

# Extract the URL from the specified <section> element with class 'links banners masonry-brick'
banner_sections = soup.find_all('section', {'class': 'links banners masonry-brick'})
...
try:
    with open('headhuntersbrazil.json', 'r') as file:
...
with open('headhuntersbrazil.json', 'w') as file:
